...
def select_daily_animal():
    ...
    global daily_animal
    random_file_path = os.path.join(animals_path, random.choice(os.listdir(animals_path)))
    ...
    try:
        ...
        if whom == 'all':
            ...
            for chat_id in chat_ids:
                try:
                    bot.send_photo(chat_id, caption=f"Сегодняшнее животное дня это: {daily_animal['name_animal']}\n\n{daily_animal['brief_info']}", photo=daily_animal['img_src'])
                    print(f"Животное дня отправлено в чат {chat_id}: {daily_animal['name_animal']}")
                except Exception as e:
                    print(f"Ошибка при отправке в чат {chat_id}: {e}")
        else:
            # Отправляем животное дня в определенный чат
            try:
                bot.send_photo(whom, caption=f"Сегодняшнее животное дня это: {daily_animal['name_animal']}\n\n{daily_animal['brief_info']}", photo=daily_animal['img_src'])
                print(f"Животное дня отправлено в чат {whom}: {daily_animal['name_animal']}")
            except Exception as e:
                print(f"Ошибка при отправке в чат {chat_id}: {e}")
    except Exception as e:
...
def schedule_animal_of_the_day(send_time="15:00"):
    schedule.every().day.at("00:00").do(select_daily_animal)  # Выбираем животное дня в полночь
    schedule.every().day.at(send_time).do(send_animal_of_the_day)  # Отправляем в указанное время
...
def start_message(message):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True) # Инициилизируем клавиатуру
    ...
    markup.add(animal_of_the_day_button, finder, random_animal, random_fact, photo_findef) # Создаем клавиатуру

    bot.send_message(message.chat.id, star_message, reply_markup=markup) # Отправляем стартовое сообщение
...
def reply_to_message(message):
    if message.text == 'Случайное животное':
        ...
        bot.send_photo(message.chat.id, caption=random_animal['brief_info'], photo=random_animal['img_src'])
        ...
        bot.send_message(message.chat.id, 'Что бы найти определенное животное напишите:\nПоиск: животное')

    elif message.text == 'Поиск фото':
        bot.send_message(message.chat.id, 'Для того что бы найти фотографии животного напишите:\nПоиск фото: животное')
        ...
        try:
            ...
            parser_viku.save_json_file(date_animal, date_animal["name_animal"], folder_file)
            parser_viku.save_img(date_animal["img_src"], date_animal["path_file"])
            bot.send_photo(message.chat.id, photo=date_animal['img_src'], caption=f'{date_animal["name_animal"]}\n\n{date_animal["brief_info"]}')

        except requests.exceptions.HTTPError:
            bot.send_message(message.chat.id, "По запросу ничего не найдено, возможно в запросе есть грамотическая ошибка")

        except requests.exceptions.ConnectTimeout:
            bot.send_message(message.chat.id, 'Ошибка с соеденением, попробуйте еще раз')
            time.sleep(3)
        ...
        try:
            ...
            paths_img = parser_foto_animal.save_img(urls_img_animal, animal.lower(), folder_static)
            parser_foto_animal.save_json_file(paths_img, name_json_file, folder_static)

            with open(f'./static/path_json_animal/{name_json_file}', 'r', encoding='utf-8') as file:
                photos_list = json.load(file)

            media = [types.InputMediaPhoto(open(photo_path, 'rb')) for photo_path in photos_list[:9]]
            bot.send_media_group(message.chat.id, media=media)

        except requests.exceptions.HTTPError:
            bot.send_message(message.chat.id, "По запросу ничего не найдено, возможно в запросе есть граматическая ошибка")
...
schedule_animal_of_the_day(send_time="05:14") # Отправляем животное дня в определенное время 
